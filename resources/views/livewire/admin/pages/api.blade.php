<div class="container" x-data="{ 'createApiTokenModal':false }" @keydown.esc="createApiTokenModal=false">
        ...
        <div class="fixed bottom-4 right-2 z-50">
            @if( session()->has('success'))
                <div
                        ...
                        x-data="{ show: true }" x-show="show" wire:key="{{ ('createApiTokenModal').random_int(3,8) }}"
    ...
    <section id="api-info-table" class="relative overflow-x-auto">
        <table class="table-auto w-full text-sm text-left text-gray-500 dark:text-gray-400">
            ...
            <tr class="bg-white border-b dark:bg-gray-800 dark:border-gray-700 m-8">
                <td >{{ config('pos-api.CLIENT_ID') }} </td>
                <td >{{ config('pos-api.USERNAME') }} </td>
                <td >{{ config('pos-api.PASSWORD') }} </td>
                <td >{{ config('pos-api.CLIENT_SECRET') }} </td>
                <td >{{ config('pos-api.GRANT_TYPE') }} </td>
                <td >{{ config('pos-api.HOST') }} </td>
                <td >{{ config('pos-api.URI') }} </td>
            </tr>
            </tbody>
        </table>
    </section>
    ...
    <section id="createApiTokenModal" tabindex="-1" role="dialog" aria-modal="true" x-show="createApiTokenModal"
             ...
             wire:key="{{ ('createApiTokenModal').random_int(3,8) }}">
                ...
                <form wire:submit.prevent="createAccessToken" class="space-y-4 p-5 lg:p-8 mx-2 mb-2 text-gray-800 dark:text-gray-200">
                    <div>
                        ...
                        <x-jet-input type="text"
                                     ...
                                     placeholder="api url" id="form.uri"
                                     wire:model.defer="form.uri"></x-jet-input>
                        <x-jet-input-error for="form.uri"></x-jet-input-error>
                    </div>
                    <div>
                        <label class="sr-only" for="form.host">API ENDPOINT</label>
                        <x-jet-input type="text"
                                     class="w-full p-3 border border-red-200 rounded-lg bg-transparent focus:ring-2 focus:ring-indigo-500 outline-none "
                                     placeholder="api url" id="form.host"
                                     wire:model.defer="form.host"></x-jet-input>
                        <x-jet-input-error for="form.host"></x-jet-input-error>
                    </div>
                    ...
                    <div>
                        ...
                        <x-jet-input-error for="form.client_secret"></x-jet-input-error>
                    </div>
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        <div>
                            ...
                            <x-jet-input type="text"
                                         class="w-full p-3 border border-red-200 rounded-lg bg-transparent focus:ring-2 focus:ring-indigo-500 outline-none "
                                         placeholder="Client ID" id="form.client_id"
                                         wire:model.defer="form.client_id"></x-jet-input>
                            <x-jet-input-error for="form.client_id"></x-jet-input-error>
                        </div>
                        <div>
                            <label class="sr-only" for="form.grant_type">Grant Type</label>
                            <x-jet-input type="text"
                                         class="w-full p-3 border border-red-200 rounded-lg bg-transparent focus:ring-2 focus:ring-indigo-500 outline-none "
                                         placeholder="Grant Type Ex: Password" id="form.grant_type"
                                         wire:model.defer="form.grant_type"></x-jet-input>
                            <x-jet-input-error for="form.grant_type"></x-jet-input-error>
                        </div>
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        <div>
                            ...
                            <x-jet-input type="text"
                                         ...
                                         wire:model.defer="form.username"></x-jet-input>
                            <x-jet-input-error for="form.username"></x-jet-input-error>
                        </div>
                        <div>
                            ...
                            <x-jet-input type="password"
                                         ...
                                         wire:model.defer="form.password"></x-jet-input>
                            <x-jet-input-error for="form.password"></x-jet-input-error>
                        </div>
                    ...
                    <div class="m-4 px-5 w-auto text-gray-800 dark:text-gray-200">
                        ...
                        <div class="flex justify-between">
                            ...
                            <p class="text-md text-gray-800 dark:text-gray-200">
                                ...
                                {{ \Carbon\Carbon::now()->addMilliseconds($token['expires_in'])->format('M d Y, H:i:s') }}</p>
                        </div>
                        <p>
                        ...
                        <p class="text-md text-red-500 dark:text-red-400 italic pb-2">
                            {{ "Store the above information securely for future use." }}
                        </p>
                    </div>
                @endif
            </div>
